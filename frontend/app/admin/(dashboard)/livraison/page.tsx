  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={onSubmit} className="mt-6 flex max-w-2xl flex-wrap gap-3 rounded-2xl bg-white p-5 shadow-sm">
        <input required placeholder="Zone *" className="rounded-xl border px-3 py-2" value={form.zone_name} onChange={(e) => setForm((f) => ({ ...f, zone_name: e.target.value }))} />
        <input placeholder="Ville" className="rounded-xl border px-3 py-2" value={form.city} onChange={(e) => setForm((f) => ({ ...f, city: e.target.value }))} />
        <input required type="number" placeholder="Frais" className="rounded-xl border px-3 py-2" value={form.delivery_fee} onChange={(e) => setForm((f) => ({ ...f, delivery_fee: e.target.value }))} />
        <input placeholder="Délai estimé" className="rounded-xl border px-3 py-2" value={form.estimated_delay} onChange={(e) => setForm((f) => ({ ...f, estimated_delay: e.target.value }))} />
